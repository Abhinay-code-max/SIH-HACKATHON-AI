...
# ==============================================================================
EXPERIMENT_ID = "U2_1_yolov8m_640_9class_v002_10ep"
...
U2_BEST_SHA = "cd8e50a9a84aef125450e81e79c8d4e5d8b1252604cd820167c5f094ac5e7930"
U2_LAST_SHA = "71f368ed1687392a86c2b906c0cdeaea116c4b97a7a051d5edc6f9055bfc40f6"
U2_YAML_SHA = "7c3cb031dbba632f783dc04aebd46a6c71ef4192f59e4e6b906839d2764faf7e"
...
def verify_immutability(stage: str = "PRE"):
    print(f"\n[Audit] Verifying D1, U1, and U2 Baseline Immutability ({stage}-TRAINING)...")
    checks = {
        ...
        "U2 best.pt": (ROOT_DIR / "training_lab/runs/U2_yolov8m_640_9class_v002/weights/best.pt", U2_BEST_SHA),
        "U2 last.pt": (ROOT_DIR / "training_lab/runs/U2_yolov8m_640_9class_v002/weights/last.pt", U2_LAST_SHA),
        "U2 data.yaml": (ROOT_DIR / "training_lab/datasets/dataset_unified_v002/data.yaml", U2_YAML_SHA),
    }
...
def execute_full_u2_training() -> int:
    ...
    # 14. Compile Acceptance Reports in training_lab/reports/U2_1_phase6/
    reports_dir = ROOT_DIR / "training_lab" / "reports" / "U2_1_phase6"
    ...
    # Generate U2_1_PHASE6_CHECKPOINT_HASHES.json
    checkpoint_hashes = {
    ...
    with open(reports_dir / "U2_1_PHASE6_CHECKPOINT_HASHES.json", "w", encoding="utf-8") as f:
        json.dump(checkpoint_hashes, f, indent=2)

    # Generate U2_1_PHASE6_METRICS.json
    metrics_report = {
        "experiment_id": EXPERIMENT_ID,
    ...
    with open(reports_dir / "U2_1_PHASE6_METRICS.json", "w", encoding="utf-8") as f:
        json.dump(metrics_report, f, indent=2)

    # Generate U2_1_PHASE6_INFERENCE_VERIFICATION.json
    inf_report = {
        "experiment_id": EXPERIMENT_ID,
    ...
    with open(reports_dir / "U2_1_PHASE6_INFERENCE_VERIFICATION.json", "w", encoding="utf-8") as f:
        json.dump(inf_report, f, indent=2)

    # Generate U2_1_PHASE6_RARE_CLASS_ANALYSIS.md
    weak_classes = ["truck", "motorcycle", "bicycle", "backpack", "bag"]
    rare_md = f"""# BORDER SENTINEL — U2.1 PHASE 6 RARE-CLASS ANALYSIS
...
| Class Name | U1 Baseline mAP50 (%) | U2.1 on V001 Benchmark mAP50 (%) | $\\Delta$ mAP50 | U1 Recall (%) | U2.1 on V001 Recall (%) | $\\Delta$ Recall |
| :--- | :---: | :---: | :---: | :---: | :---: | :---: |
"""
...
"""
    with open(reports_dir / "U2_1_PHASE6_RARE_CLASS_ANALYSIS.md", "w", encoding="utf-8") as f:
        f.write(rare_md)

    # Generate U2_1_PHASE6_BASELINE_COMPARISON.md
    base_comp_md = f"""# BORDER SENTINEL — U2.1 PHASE 6 BASELINE COMPARISON
**Experiment**: `{EXPERIMENT_ID}`  
**Timestamp**: `{datetime.now(timezone.utc).isoformat()}`  
...
| Metric | U1 on V001 Validation | U2 on V002 Validation (5 Ep) | U2.1 on V002 Validation (10 Ep) | U2.1 on V001 Benchmark |
| :--- | :---: | :---: | :---: | :---: |
| **Precision** | {u1_v001_val['precision']:.2f}% | 52.06% | {v002_val_metrics['all']['precision']:.2f}% | {v001_comp_metrics['all']['precision']:.2f}% |
| **Recall** | {u1_v001_val['recall']:.2f}% | 43.59% | {v002_val_metrics['all']['recall']:.2f}% | {v001_comp_metrics['all']['recall']:.2f}% |
| **mAP50** | {u1_v001_val['mAP50']:.2f}% | 42.58% | {v002_val_metrics['all']['mAP50']:.2f}% | {v001_comp_metrics['all']['mAP50']:.2f}% |
| **mAP50-95** | {u1_v001_val['mAP50_95']:.2f}% | 25.42% | {v002_val_metrics['all']['mAP50_95']:.2f}% | {v001_comp_metrics['all']['mAP50_95']:.2f}% |
...
"""
    with open(reports_dir / "U2_1_PHASE6_BASELINE_COMPARISON.md", "w", encoding="utf-8") as f:
        f.write(base_comp_md)

    # Generate U2_1_PHASE6_TRAINING_REPORT.json
    full_training_report = {
        ...
        "immutability_audit": {
            ...
            "U2_best_sha_verified": True,
            "U2_last_sha_verified": True,
            "U2_yaml_sha_verified": True,
        }
    }
    with open(reports_dir / "U2_1_PHASE6_TRAINING_REPORT.json", "w", encoding="utf-8") as f:
        json.dump(full_training_report, f, indent=2)

    # Generate U2_1_PHASE6_TRAINING_SUMMARY.md
    summary_md = f"""# BORDER SENTINEL — U2.1 PHASE 6 TRAINING SUMMARY
**Experiment**: `{EXPERIMENT_ID}`  
...
============================================================
U2.1 TRAINING STARTED            = YES
U2.1 TRAINING COMPLETED          = YES
U2.1 BEST CHECKPOINT             = {best_pt}
U2.1 BEST CHECKPOINT SHA256      = {best_pt_sha256}
U2.1 VALIDATION mAP50            = {v002_val_metrics['all']['mAP50']:.2f}%
U2.1 VALIDATION mAP50-95         = {v002_val_metrics['all']['mAP50_95']:.2f}%
U2.1 HOLDOUT USED DURING TRAINING = NO
U1 PROTECTED HOLDOUT LEAKAGE     = NONE
D1 IMMUTABILITY                  = PASS
U1 IMMUTABILITY                  = PASS
U2 IMMUTABILITY                  = PASS
...
"""
    with open(reports_dir / "U2_1_PHASE6_TRAINING_SUMMARY.md", "w", encoding="utf-8") as f:
        f.write(summary_md)

    print("\n" + "=" * 80)
    print("U2.1 TRAINING & ACCEPTANCE REPORTS GENERATION COMPLETE")
    print("=" * 80)
    return 0
